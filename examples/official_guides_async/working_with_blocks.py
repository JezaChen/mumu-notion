...
from examples.official_guides_async.data import BlockExampleData
...
async def run_example_code(is_continuous=False):
    ...
    block_append_rsp = await client.blocks.children.append(base_page_id,
                                                           BlockExampleData.CHILD_BLOCK_APPEND_DATA_EXAMPLE)
    ...
    list_rsp = await client.blocks.children.list(base_page_id)
    ...
    retrieve_rsp = await client.blocks.retrieve(appended_heading2_block_id)
    ...
    update_rsp = await client.blocks.update(appended_heading2_block_id,
    ...
    delete_rsp = await client.blocks.delete(appended_paragraph_block_id)
    print(f"--- Paragraph Block Delete Result ---\n"
          f"{delete_rsp}\n"
          f"--- Paragraph Block Delete Result ---")

    delete_rsp = await client.blocks.delete(appended_heading2_block_id)
