...
from examples.official_guides_async.data import DatabaseExampleData
...
async def run_example_code(is_continuous=False):
    ...
    meal_database_create_rsp = await client.databases.create(meal_database_create_body_data)
    ...
    grocery_database_create_rsp = await client.databases.create(grocery_database_create_body_data)
    ...
    grocery_database_retrieve_rsp = await client.databases.retrieve(grocery_database_id)
    ...
    for example_data in example_data_list:
        await client.pages.create(example_data)
    ...
    search_rsp = await client.databases.query(grocery_database_id,
                                        {
                                            "filter": {
                                                "property": "Name",
                                                "title": {"equals": "Mango"}
    ...
    if len(search_rsp["results"]):
        ...
        first_result_retrieve_rsp = await client.pages.retrieve(first_result_id,
    ...
    for page_info in search_rsp["results"]:
        ...
        prop_retrieve_rsp = await client.pages.properties.retrieve(page_id,
                                                             prop_id)
    ...
    for page_info in search_rsp["results"]:
        ...
        prop_update_rsp = await client.pages.update(page_id,
                                              {
                                                  "properties": {
                                                      "Description": {
                                                          "rich_text": [
    ...
    database_update_rsp = await client.databases.update(grocery_database_id, DatabaseExampleData.GROCERY_UPDATE_EXAMPLE)
    ...
    for page_info in search_rsp["results"]:
        page_id = page_info["id"]
        await client.blocks.delete(page_id)
    ...
    await client.blocks.delete(grocery_database_id)
    await client.blocks.delete(meal_database_id)
